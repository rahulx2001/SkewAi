...
def _build_clusters(pack_id: str) -> list[dict]:
    ...
    return [
        {"cluster_id": 41, "pack_id": pack_id, "top_terms": json.dumps(["double", "charged", "chase"]),
         ...
         "first_seen": _NOW - timedelta(days=90), "last_seen": _NOW - timedelta(days=5)},
        {"cluster_id": 52, "pack_id": pack_id, "top_terms": json.dumps(["unauthorized", "fraud", "credit card"]),
         ...
         "first_seen": _NOW - timedelta(days=200), "last_seen": _NOW - timedelta(days=15)},
...
def _build_cluster_assignments() -> list[dict]:
    out = []
    for r in _RECORDS:
        if r["record_id"].startswith("CFPB-4000"):
            cluster = 41
        elif r["record_id"].startswith("CFPB-5000"):
            cluster = 52
        else:
            cluster = 63
